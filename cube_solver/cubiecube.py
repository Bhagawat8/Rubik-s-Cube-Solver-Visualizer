...
# n choose k - optimized version using pre-computed table
def Cnk(n, k):
    """Fast binomial coefficient - falls back to calculation if not pre-computed"""
    try:
        from .bit_tables import fast_cnk
        result = fast_cnk(n, k)
        if result > 0:
            return result
    except ImportError:
    ...
    # Fallback to calculation if pre-computed table not available
    if n < k:
        return 0
    if k > n // 2:
        k = n - k
...
class CubieCube(object):
    ...
    def cornerMultiply(self, b):
        ...
        Legacy note: certain helpers are only used by the high-level DCP search.
